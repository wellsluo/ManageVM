...
if(-not $prompt) {
    Write-Host "    None, $($vmName) is already setup for nesting"
    Exit;
}
...
$char = 'Y'  #modify to enable it automatically. By Wei Luo
...
if($char.StartsWith('Y')) {
    ...
    <#    Write-Host "Would you like to enable? (Y/N)" -NoNewline
        $input = Read-Host
    #>
        $input = 'Y'   #modify to enable it automatically. By Wei Luo
        if($input -eq 'y' -or 'Y') {
            Set-VMNetworkAdapter -VMName $vmName -MacAddressSpoofing on
            Write-Host "Enabled Mac Address Spoofing"
        }
        else {
